...
pub const MetalDevice = struct {
    ...
    const bridge = struct {
        ...
        // Command submission
        extern fn guava_metal_rhi_submit(ctx: *anyopaque, queue_class: u32, cmd_bytes: [*]const u8, cmd_len: u32) bool;
        // Swapchain
...
fn submitGraphics(ctx: *anyopaque, cmd: *const command_buffer.CommandBuffer, desc: queue_mod.SubmitDesc) !void {
    ...
    const bytes = cmd.rawBytes();
    if (bytes.len == 0) return;
    if (!MetalDevice.bridge.guava_metal_rhi_submit(self.bridge_ctx, 0, bytes.ptr, @intCast(bytes.len)))
        return error.SubmitFailed;
}
...
fn submitCompute(ctx: *anyopaque, cmd: *const command_buffer.CommandBuffer, desc: queue_mod.SubmitDesc) !void {
    ...
    const bytes = cmd.rawBytes();
    if (bytes.len == 0) return;
    if (!MetalDevice.bridge.guava_metal_rhi_submit(self.bridge_ctx, 1, bytes.ptr, @intCast(bytes.len)))
        return error.SubmitFailed;
}
...
fn submitTransfer(ctx: *anyopaque, cmd: *const command_buffer.CommandBuffer, desc: queue_mod.SubmitDesc) !void {
    ...
    const bytes = cmd.rawBytes();
    if (bytes.len == 0) return;
    if (!MetalDevice.bridge.guava_metal_rhi_submit(self.bridge_ctx, 2, bytes.ptr, @intCast(bytes.len)))
        return error.SubmitFailed;
}
