...
public final class PanelRegistry {
    ...
    public func updateDescriptor(id: PanelID,
    ...
    public func unregister(id: PanelID) {
        byID.removeValue(forKey: id)
        order.removeAll { $0 == id }
    }

    public func descriptor(for id: PanelID) -> PanelDescriptor? {
        byID[id]
    }

    /// 返回与 id 对应的视图；缺失时返回 `EmptyView`，由调用方决定是否提示。
    public func make(_ id: PanelID) -> AnyView {
    ...
    public var workspaceRegistry: WorkspacePanelRegistry {
        WorkspacePanelRegistry(descriptors.map(\.workspaceDescriptor))
    }
}
