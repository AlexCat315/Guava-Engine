...
display timing/log/runtime inventory data, and forward pointer, wheel, text, and
common keyboard input through the mirror. Selecting a node sends `select.node`
with the stable `elementID`, which the host uses to draw a runtime overlay.
