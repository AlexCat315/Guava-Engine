...
// Improved normal reconstruction using 5-tap best-fit method
// Avoids edge artifacts from simple dFdx/dFdy
vec3 reconstructNormal(vec3 viewPos, vec2 uv) {
    ...
    // Choose the pair with the smallest depth difference (more accurate on edges)
...
// Cosine-distributed hemisphere kernel (32 precomputed samples)
...
void main() {
    ...
    for (int i = 0; i < kernelSize; ++i) {
        ...
        // Improved range check: smooth falloff based on distance to sample
    ...
    // Cross-bilateral blur (4-tap) for noise reduction without losing edges
    float blur_ao = ao;
    ...
    for (int x = -1; x <= 1; x += 2) {
        for (int y = -1; y <= 1; y += 2) {
            vec2 sample_uv = v_uv + vec2(float(x), float(y)) * texel;
            ...
            vec3 nPos = getViewPos(sample_uv, neighbor_depth);
            float recomputed_ao_approx = ao; // approximation for bilateral weight
            blur_ao += ao * w; // simplified bilateral
            total_weight += w;
        }
    }
    blur_ao /= total_weight;

    out_ao = mix(1.0, blur_ao, ssao.u_intensity);
}
