//!guava builtin=fly_camera move_speed=5.0 mouse_sensitivity=0.002
...
pub fn onInit(ctx: *context.ScriptContext) void {
    ...
    const data = ctx.allocator.create(CameraData) catch {
        ctx.logError("Failed to allocate CameraData");
        return;
    };
    data.* = CameraData{};
...
pub fn onUpdate(ctx: *context.ScriptContext, dt: f32) void {
    ...
    const data = ctx.getUserData(CameraData) orelse {
        ctx.logError("CameraData not found");
        return;
    };

    // 获取Transform
    const transform = ctx.getTransform() orelse {
        ctx.logError("Failed to get transform");
        return;
    };
    ...
    if (ctx.wasKeyPressed(input_mod.Key.tab)) {
        ...
        ctx.log("Camera pos: ({d:.2}, {d:.2}, {d:.2}), FPS: {d:.1}, Time: {d:.2}", .{ pos[0], pos[1], pos[2], fps, ctx.getTime() });
    }
}
