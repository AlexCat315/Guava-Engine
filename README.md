...
- Phase 6 的 Scene / Prefab / Material 保存-读取-再保存一致性测试已补齐；WASM 公共变量也已反射进 Inspector，可直接灰盒调参并热应用到运行中的 WasmVM 实例
- 当前剩余主任务集中在脚本资产正式文本 schema / 持久化、Editor Utility UI、Query 扩展，以及更重的 headless / ECS 内核演进
