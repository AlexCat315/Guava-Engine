...
- MCP `stdio` 已支持场景资源、实体写工具，以及协作资源：`scene://hierarchy`、`selection://current`、`entity://{id}`、`schema://components`、`editor://context`、`editor://intent-log`、`preview://staged`
...
- 现已具备首版 AI 规则与发现层：`schema://components` 可读，MCP `query_entities` 支持 `count_only` / `limit` / `offset` / `truncated`
- 当前剩余主任务集中在更完整的 schema 资源族、WASM 参数反射 / 灰盒逻辑 UI，以及更重的 Query / headless / ECS 内核演进
...
- AI 与 UI 已开始共用命令总线和 staged transaction；当前缺口主要在更深的 viewport 材质混合、脚本生成、查询层，而不是 MCP 协议本身
