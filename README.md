...
- 路径追踪重写：CPU / Metal 已同步 GGX VNDF + NEE / MIS + Principled BSDF + HDR `.hdr` 环境重要性采样 + 俄罗斯轮盘 + 8x8 tile adaptive sampling；Editor PNG 导出已支持 `albedo / normal` AOV sidecar + 自动降噪后端 `OIDN(动态加载) -> MPS Guided -> CPU Guided`，序列导出仍待补
