...
- 引擎级 `CommandQueue` 已落地最小闭环，Inspector / Hierarchy / 基础创建路径已复用；MCP 已支持 `stage/apply/discard` staged transaction，编辑器 viewport 已有 ghost preview pins、apply/discard overlay、同视口 staged wireframe ghost pass，并支持直接选中 ghost 后用 gizmo 调整 staged transform
- WASM 脚本 backend、语义查询层、真正的双世界材质/半透明混合预览仍未完成
