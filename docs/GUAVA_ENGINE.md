...
**基础建模快捷键（2026-04 当前实现）**
- 模式与选择:
    - `Tab`: Object/Edit Mode 切换
    - `1 / 2 / 3`: 点/边/面选择模式
    - `A`: 当前元素模式全选/取消全选
    - `Alt + 左键`（边模式）: 环形边循环选择（edge loop）
- 核心编辑:
    - `E` 或 `Alt+E`: 挤出（当前为直接进入 modal；高级挤出菜单待实现）
    - `Ctrl+R`: 环切（modal，鼠标控制 slide factor）
    - `B` 或 `Ctrl+B`: 倒角（modal）
    - `I`: 内插（modal）
    - `M`: 合并顶点
    - `F`: 填面（当前支持由 3 个点/边推导三角面）
    - `P`: 分离面
    - `Shift+N`: 重算法线
    - `.`: 轴心到选中
- 当前未实现（规划中）:
...
**Phase 1: 参数材质增强**
- [x] 统一 Principled 参数面板: BaseColor / Metallic / Roughness / Normal / Emissive / Alpha
- [x] 预览球体、预览平面、贴图 checker（MVP: 快捷切换与快速应用）
- [x] 材质实例与共享母材质（共享检测 + 一键独立实例）

**2026-04 当前实现细节（Phase 1）**
- [x] 参数面板: Base Color / Opacity / Metallic / Roughness / Emissive / Alpha Cutoff / Double Sided
- [x] 纹理槽位: Base Color / Metallic-Roughness / Normal / Occlusion / Emissive
- [x] 预览便捷操作: 一键切换 Sphere/Plane 预览网格、Apply Checker To Base Color
- [x] 防误改: 材质共享计数显示、共享材质提示、`Make Unique Instance`
- [x] AST 基础: `material_ast.zig` + `MaterialAst.fromResource/toResourceDesc`
- [x] AST 写回链路: Material Editor 变更统一经 AST 回写 MaterialResource
- [x] AST 读桥接: Raster (`mesh_pass.resolveMaterial`) 与 Path Trace（材质 hash + 纹理索引）已接 AST 入口

**当前未完成（Phase 1 后续）**
- [x] 更完整纹理通道 UX（每槽位缩略图、通道说明、缺失资源提示）
- [x] 材质实例继承关系可视化（母材质/实例层级摘要）
- [ ] 材质实例回退与覆盖对比

**2026-04 本轮补齐**
- [x] 独立预览子场景：Material Editor 现改为使用独立离屏预览场景与专用预览贴图，不再通过修改选中实体的 mesh 来切换球体/平面预览
- [x] Preview sync 升级为完整 Phase 1 材质镜像：预览场景现同步 Shading / BaseColor / Metallic / Roughness / Emissive / AlphaCutoff / DoubleSided / IBL，以及五个纹理槽
- [x] 纹理槽位 UX：每个槽位增加实时缩略图、语义说明、空槽位 fallback 提示与丢失资源告警
- [x] 材质实例父链：共享材质克隆为实例时写入 parent handle / name hint / generation，并在 Material Editor 中展示实例深度与父材质摘要

**Phase 2: 节点式材质编辑器**
- [ ] 节点系统: PBR 参数节点 -> 输出节点
- [ ] 内置节点: Texture Sample, Color, Float, Mix, Normal Map, Noise, Voronoi
- [ ] 编译为光栅 Uber Shader 参数图 + Path Tracer BSDF 闭包
- [x] 节点底座：`MaterialAst` / `MaterialResource` 现支持可选 `MaterialGraph`，并在扁平参数写回时自动生成 canonical graph seed
...
- [x] AudioSource / AudioListener / AudioClip 组件
- [x] 3D 空间音效: 距离衰减、Voice group / bus 路由
- [x] Inspector 编辑: 音量 / 循环 / 3D 设置 / bus
- **验收**: ✅ 错误诊断、场景音频组件、3D 参数、Inspector 与总线路由已打通

#### GR-2 多场景管理
- [x] SceneManager: `loadScene("level_2")` / `unloadScene()`
- [x] 异步加载 + 加载界面回调
- [x] 全局不销毁对象标记 (`DontDestroyOnLoad`)
- [x] 脚本宿主桥接: `loadScene` / `unloadScene` / `setDontDestroyOnLoad` / `isSceneLoading`
- **验收**: ✅ Runtime SceneManager、异步文件读取回调、`DontDestroyOnLoad` 保留链路与 unload 保留行为已由测试覆盖
