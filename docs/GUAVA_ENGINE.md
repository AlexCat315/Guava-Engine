...
| C# Script VM | 🚧 规划中，定位为 NPC / UI / Gameplay 逻辑 |
...
- [ ] 运动向量 Pass：当前仍无正式 velocity buffer，动态物体 rejection 仍待补
- [x] 渲染顺序已放在 Tonemap 之前
- **验收**: ⚠️ 静态场景与相机移动稳定性已显著改善；快速动态物体仍需 velocity buffer 才能彻底收尾
...
#### R-7 SSR 粗糙度模糊
- [x] 已有 SSR shader / pass 原型：从 depth 重建 view-space 并做 screen-space ray march
- [ ] renderer 正式接回 HDR 主链（当前仍是 prototype 调用，未完成有效输出）
- [ ] 根据 roughness 对 SSR 结果做 cone tracing 模糊 / mip blur
- [ ] 光滑表面清晰反射，粗糙表面模糊反射
- **验收**: ❌ 当前尚未达到正式验收标准；本轮已把状态从文档中过度乐观的描述修正为待继续实施
...
实施清单:
- [ ] 接入 `C# Script VM`，优先覆盖 NPC / UI / Gameplay 生命周期
- [ ] 保持 `WASM VM` 轻量化，只暴露插件所需的宿主 API
- [ ] 编辑器中明确区分 `Scripts` 与 `Plugins` 的加载、热重载与调试入口
- [ ] 统一宿主桥接层，由 `Zig Core` 同时服务 `C# Script VM` 和 `WASM VM`
...
> SSAO 已统一为 compute 路径，TAA 使用 jitter + history resolve；SSR 仍保留 shader / pass 原型，待正式接回 HDR 并补 roughness blur。

| ID | 任务 | 检验标准 | 状态 |
|----|------|---------|------|
| R-5 | TAA (Compute temporal resolve) | 围栏边缘无闪烁 | ⚠️ |
...
| `ScriptVM` | 文档中的泛称；当前规划拆分为 `C# Script VM`（gameplay）与 `WASM VM`（plugins） |
