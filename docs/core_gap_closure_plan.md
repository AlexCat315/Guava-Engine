...
- **P7**: 剔除、BVH与射线检测重构 - 主场景 Scene Extraction 已复用 renderable BVH，并加入动态分区 BVH；完整增量更新仍未完成
...
- 已加入“被移动过的 renderable 进入动态分区 BVH”的第一版分层，避免高频拖拽持续触发静态 BVH 全量重建，也避免动态对象查询退化成线性扫。
