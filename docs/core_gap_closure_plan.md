...
- **P7**: 剔除、BVH与射线检测重构 - 主场景 Scene Extraction、Raycast 与碰撞可视化已复用 renderable BVH，并加入动态分区 BVH、叶 refit、分区缓存局部同步与稳定后回收；树内插删级增量更新仍未完成
...
- `BVH` 的更细粒度增量更新仍未完成，但已加入静态/动态 renderable 分区缓存与脏实体局部同步，避免树 dirty 时重新线性扫描整个 `World`；碰撞可视化已改为复用 bounds/BVH，其它可见性与调试路径复用仍未封账。
