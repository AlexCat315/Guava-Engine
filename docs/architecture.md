...
`Application -> World -> Window -> Renderer -> MeshPass -> RHI Device`
...
- `MeshPass` 会遍历世界资源，把 `MeshResource / MaterialResource / TextureResource` 上传成 GPU 资源并缓存。
...
2. 把 `MeshPass` 拆成更明确的 `BasePass / ShadowPass / DepthPrepass / SelectionPass`
   这样才适合编辑器和镜头工作流。
