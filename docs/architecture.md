...
- `SelectionReadback` 现在是基于 `SDL_GPUFence` 的异步读回队列：点击时只记录请求并在当前帧提交 `SDL_DownloadFromGPUTexture`，随后在后续帧轮询 fence，把完成的 1x1 像素解码成选中实体。
- `SelectionHistory` 负责维护多选快照、主选中对象以及后续编辑器可用的 undo/redo 基础。
- `DepthPrepass` 先建立深度，给后续基础材质和编辑器辅助 pass 复用。
- `BasePass` 负责当前最小 mesh/material 提交路径。
- `OutlinePass` 负责基于 ID 纹理做最终轮廓高亮合成，当前已经支持多个选中实体逐个叠加轮廓。
...
- 鼠标左键点击窗口时，`Renderer` 会发起一次异步 `SelectionReadback`；普通点击替换选择，`Shift/Ctrl/Cmd + Click` 会走多选 toggle。
- 当前选择状态不再只是单个实体，而是 `SelectionHistory` 里的快照；轮廓高亮会覆盖当前所有被选中的 mesh 实体。
...
2. 把当前 pass 继续扩到 `ShadowPass / GizmoPass / EditorComposite / SelectionReadback Visualization`
