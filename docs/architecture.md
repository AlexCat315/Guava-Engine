...
- `src/engine/render`
  Renderer、RenderGraph 和 MeshPass，负责把场景快照变成一组明确的渲染阶段。
- `src/engine/rhi`
...
`Application -> Window -> Renderer -> MeshPass -> RHI Device`

- `Application` 管生命周期、Layer、主循环。
- `Window` 只管 SDL3 窗口和平台事件。
- `Renderer` 管 RenderGraph、场景快照和每帧提交策略。
- `MeshPass` 管当前最小 mesh 管线、shader、纹理采样、材质 uniform 和 draw call 组织。
...
- `Scene.resources` 现在持有明确的 `MeshResource / MaterialResource / TextureResource`，渲染层不再依赖硬编码几何体。
- `glTF 2.0` 静态导入已经接进 `Scene.importGltfStaticModel()`，当前覆盖网格、节点层级、TRS、顶点色、UV 和 `baseColorFactor`。
...
- `MeshPass` 会遍历场景资源，把 `MeshResource / MaterialResource / TextureResource` 上传成 GPU 资源并缓存。
- 默认示例场景会同时画内建 plane/cube 和一个导入的 glTF 金字塔。
...
2. 把 glTF 导入继续扩到 image decode、法线、切线和多 primitive/material
   这样才能更接近 DCC 和影视预览资产。
3. 把 `MeshPass` 拆成更明确的 `BasePass / ShadowPass / DepthPrepass / SelectionPass`
   这样才适合编辑器和镜头工作流。
4. 把 Scene 升级成更明确的 World/Entity/Asset 边界
   这样更适合批处理、镜头组织和工具链。
5. 再往上做编辑器和 DCC/影视工具链接口
