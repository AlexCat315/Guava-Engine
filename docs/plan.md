...
**状态**: 协作基础设施、第二世界着色预览、首版 WASM 脚本闭环、schema / query / 参数反射已落地；剩余集中在脚本持久化收口、Editor Utility UI、Query 扩展与内核演进
...
| 2 | **脚本资产 schema / 持久化** | 让 Script 组件与参数进入稳定文本资源与保存链路 |
| 3 | **Editor Utility UI** | 让 AI 生成编辑器专用面板，而不只是生成行为脚本 |

### 🟡 中优先级 (提升开发体验)

| 优先级 | 任务 | 描述 |
|--------|------|------|
| 4 | **AI 脚本 / 查询层** | 语义查询扩展、脚本资产规则暴露、脚本验证闭环 |
