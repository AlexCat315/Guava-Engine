...
**状态**: 协作基础设施、第二世界着色预览和首版 WASM 脚本闭环已落地；schema / query / 参数反射仍待完成
...
| Wasm3 接入与 `WasmVM` backend | `build.zig` / `src/engine/script/wasm_vm.zig` | ✅ |
...
| 优先级 | 任务 | 描述 |
|--------|------|------|
| 1 | **Schema 资源与 Query API** | `schema://...`、分页查询、AI 验证闭环 |
| 2 | **动画图作者工具 / Blend Tree** | 资源化状态机编辑；1D/2D Blend Tree |
| 3 | **WASM 参数反射 / 灰盒逻辑 UI** | 从“能编译运行”推进到“人类可直接微调参数” |

### 🟡 中优先级 (提升开发体验)

| 优先级 | 任务 | 描述 |
|--------|------|------|
| 4 | **AI 脚本 / 查询层** | Editor Utility UI、WASM 参数暴露、语义查询扩展 |
