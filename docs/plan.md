...
1. 致命缺失 (Fatal Omissions)
*没有这些功能，无法完成任何类型的游戏闭环。*

### 1.1 音频系统 (Audio System) - **完成度: 0%**
*   **缺失组件**: `AudioSource` (发声体), `AudioListener` (听者), `AudioClip` (资源库)。
*   **功能缺失**: 
    *   不支持 WAV/MP3/OGG 播放。
    *   缺少 3D 空间音效（距离衰减、多普勒效应）。
    *   缺少混音器 (Mixer) 和音量组控制（主音量/音乐/音效）。
*   **建议方案**: 集成 **OpenAL-soft** 或 **SoLoud**。

### 1.2 游戏内 UI 系统 (Game UI/Canvas) - **完成度: 5% (仅编辑器 UI)**
*   **现状**: 只有基于 ImGui 的编辑器界面，无法用于游戏运行时的 HUD。
*   **缺失组件**: 
    *   **Canvas 系统**: 处理不同分辨率下的 UI 缩放与对齐。
    *   **核心控件**: 按钮、进度条（血条）、文本渲染（SDF 字体支持）、图片/九宫格。
    *   **交互逻辑**: 独立的 UI 输入捕获，防止点击 UI 时触发游戏内攻击。
*   **建议方案**: 实现一套轻量级的顶点缓存 UI 系统，复用已有的渲染管线。

### 1.3 场景管理与加载流 (Scene/Level Management) - **完成度: 20%**
*   **现状**: 只能加载单一 JSON 场景。
*   **缺失功能**:
    *   **场景切换 API**: `SceneManager.load("level_2")`。
    *   **异步加载**: 防止加载大地图时游戏卡死（Loading 界面）。
    *   **叠加加载 (Additive Loading)**: 允许在一个关卡中动态加载/卸载子区域。
    *   **全局对象**: 跨场景不销毁的对象（如玩家数据、全局管理器）。

### 1.4 游戏生命周期管理 (Game Loop & State) - **完成度: 0%**
*   **缺失功能**:
    *   **状态机**: `GameStart`, `Paused`, `GameOver`, `Quit`。
    *   **脚本生命周期**: 明确的 `onStart()`, `onUpdate()`, `onDestroy()` 回调顺序。
    *   **时间控制**: 时间缩放（Time Scale），用于实现暂停或慢动作效果。

---

## 2. 架构缺失 (Architectural Gaps)
*这些缺失决定了游戏的上限（性能、多人能力、发布能力）。*

### 2.1 资源分发与打包 (Asset Cooking) - **完成度: 0%**
*   **现状**: 直接读取原始文件（JSON, GLTF, PNG）。
*   **缺失功能**:
    *   **打包器 (Packer)**: 将资源压缩并打包进一个加密的二进制文件（如 `.guava` 或 `.pak`）。
    *   **平台适配**: 自动转换纹理为特定平台的压缩格式（如 BC7/ASTC）。
    *   **依赖管理**: 仅打包场景引用的资源，去除无用资源。

### 2.2 网络与多人同步 (Networking) - **完成度: 0%**
*   **缺失功能**:
    *   **传输层**: ENet/WebRTC 的封装。
    *   **同步逻辑**: 变量同步 (Replication)、RPC (远程过程调用)、客户端预测。
    *   **高层 API**: 处理玩家加入、退出、房间管理。

### 2.3 任务调度系统 (General Purpose Job System) - **完成度: 10%**
*   **现状**: 只有针对特定功能的线程（如 MCP）。
*   **缺失功能**:
    *   **Task Graph**: 允许逻辑跨核并行执行（如数千个 AI 的逻辑更新）。
    *   **数据竞争检查**: 保证多线程执行脚本逻辑的安全性。

---

## 3. 游戏性系统 (Gameplay Systems)
*这些是让游戏“好玩”和“可交互”的基础。*

### 3.1 导航与寻路 (Navigation/Pathfinding) - **完成度: 0%**
*   **缺失功能**:
    *   **NavMesh 生成**: 静态/动态网格烘焙。
    *   **寻路代理**: AI 自动避障、路径点追踪。
*   **建议方案**: 集成 **Recast/Detour**。

### 3.2 物理查询 API (Physics Query) - **完成度: 40%**
*   **现状**: 只有基础的碰撞模拟。
*   **缺失功能**:
    *   **脚本接口**: 暴露射线检测 (`Raycast`)、球体重叠检测 (`OverlapSphere`)。
    *   **触发器事件**: 脚本无法方便地接收 `onTriggerEnter`（如走进传送阵）。

### 3.3 动画事件系统 (Animation Events) - **完成度: 10%**
*   **现状**: 仅能播放动画。
*   **缺失功能**:
    *   **关键帧回调**: 动画播放到某一帧时触发脚本（如攻击判定开始、脚步声播放）。
    *   **动画融合树**: 复杂的移动状态转换（Idle -> Walk -> Run 的平滑过渡）。

---

## 4. 开发工具与调试 (DevOps & Debugging)
*影响开发者的维护效率。*

*   **存档系统 (Save System)**: 缺少持久化全局状态的框架。
*   **输入映射 (Input Mapping)**: 缺少“动作-按键”映射（如将 `Space` 和 `Gamepad A` 都映射为 `Jump`）。
*   **性能分析器 (In-game Profiler)**: 缺少在游戏内查看每一帧瓶颈的 UI 覆盖层。
*   **本地化 (Runtime Localization)**: 缺少游戏运行时的多语言文本切换
...
**待完善**:
- [x] SSAO (屏幕空间环境光遮蔽) - 基础实现完成
- [x] SSR (屏幕空间反射) - 基础实现完成
- [x] TAA (时域抗锯齿) - 基础实现完成
- [x] DOF (景深) - 基础实现完成
- [x] 点光 Cube Shadow - 基础实现完成
...
| 分页实体查询 `query_entities`（含半径 + AABB 空间过滤） | `src/engine/core/query_engine.zig` / `src/engine/mcp/tools.zig` / `src/engine/mcp/server.zig` | ✅ |
...
| 优先级 | 任务 | 描述 | 状态 |
|--------|------|------|------|
| 1 | **动画图作者工具 / Blend Tree** | 资源化状态机编辑；1D/2D Blend Tree | ✅ BlendTree 基础实现完成 |
| 2 | **Editor Utility UI** | 让 AI 生成编辑器专用面板，而不只是生成行为脚本 | ✅ ImGui API 已扩展至 35 个 native symbols |
| 3 | **脚本错误定位 / Inspector 深化** | 细化 `source_location` 回传，并把脚本参数编辑继续向更完整的工具链推进 | ✅ SourceLocation + Inspector 反射已完成 |
...
- ✅ Rigidbody 组件已迁移到 SparseSet，进入混合期
- ✅ BoxCollider/SphereCollider 组件已迁移到 SparseSet，进入混合期
