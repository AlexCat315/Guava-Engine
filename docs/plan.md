...
| Viewport staged wireframe ghost pass | `src/engine/render/renderer.zig` / `base_pass.zig` / `mesh_pass.zig` | ✅ |
...
| 2 | **双世界渲染混合深化** | 当前已进入同视口 wireframe ghost pass；后续仍需补材质化、半透明和更强的双世界混合 |
| 3 | **动画图作者工具 / Blend Tree** | 资源化状态机编辑；1D/2D Blend Tree |

### 🟡 中优先级 (提升开发体验)

| 优先级 | 任务 | 描述 |
|--------|------|------|
| 4 | **AI 脚本 / 查询层** | WASM 逻辑生成、参数反射、语义查询 API |
