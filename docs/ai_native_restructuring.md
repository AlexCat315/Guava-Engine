...
10. `build.zig` 已接入 Wasm3；`src/engine/script/wasm_vm.zig`、`src/engine/script/wasm_compiler.zig`、`src/engine/mcp/tools.zig` 已打通 `compile_script` 与 `script://runtime-status`。
...
| WASM 脚本 VM | 已落地首版：Wasm3 / `WasmVM` / `wasm_compiler` / hot reload / `script://runtime-status` |
| 语义查询 API | 不存在 |
| `schema://...` 资源 | 不存在 |
