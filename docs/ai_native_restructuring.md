...
以下是 Guava Engine 在 AI 支持（尤其是 Vibecoding 体验）上的核心缺失：

### 1. 缺乏“场景感知”能力 (Lack of Scene Context)
*   **现状**: AI 只读代码，不读“场景”。它知道你的 `Player.zig` 怎么写，但它不知道场景里有一个名为 `Boss_Dragon` 的模型，也不知道这个模型离玩家有多远。
*   **Vibecoding 要求**: 你应该能直接对 AI 说：“让那个红色的球在玩家靠近时跳起来。”
*   **缺失**: 缺少一个实时将 **3D 场景树 (Scene Hierarchy)** 和 **空间属性** 向量化并喂给 LLM 的上下文窗口。
* **缺失**: 缺少一个将场景快照发给 AI 的接口，缺少 AI 能够直接操作场景的接口。

### 2. 缺乏“即时反馈循环” (No Hot-Reloading Loop for AI)
*   **现状**: AI 生成一段代码，你可能需要手动编译、运行，发现错后再反馈。
*   **Vibecoding 要求**: AI 修改逻辑后，游戏不重启，效果立刻在视口（Viewport）中呈现。
*   **缺失**: 尽管有 WASM，但缺乏一个专为 AI 设计的、具备极高容错率的**实时脚本热重载沙盒**。AI 写错一点语法，整个引擎不应该崩溃。

### 3. 缺乏“多模态视觉生成”集成 (No Multimodal Visual Integration)
*   **现状**: 你不能让 AI 帮你画贴图、调材质或写复杂的 Shader。
*   **Vibecoding 要求**: “把这个材质调成生锈的金属质感，带点雨水滑落的效果。”
*   **缺失**: 
    *   没有集成 Stable Diffusion/ControlNet 自动生成贴图。
    *   没有基于自然语言的 **Shader Graph 自动生成器**。

### 4. 缺乏“游戏逻辑原子化” (Granular Action API)
*   **现状**: AI 面对的是底层的 Zig 代码，这对 LLM 来说推理负担太重。
*   **Vibecoding 要求**: 引擎应提供一套“原子化”的高层指令集。例如 `MoveTo`, `PlayAnim`, `SpawnEffect`。
*   **缺失**: 引擎目前的 API 太碎、太底层，AI 很难一次性写出完全正确的、复杂的逻辑闭环。

### 5. 缺乏“自动化 QA 与自愈” (No Self-Healing/Auto-QA)
*   **现状**: AI 写出 Bug，你得自己调。
*   **Vibecoding 要求**: AI 运行代码，引擎报错，AI 自动捕获错误日志并修复，直到运行成功。
*   **缺失**: 一个全自动的 **“生成 -> 运行 -> 捕获报错 -> 自我修正”** 的闭环工作流。
