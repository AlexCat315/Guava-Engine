...
> 状态：执行计划 v4，已同步当前已落地的 MCP 可写闭环、编辑器上下文总线、staged collaboration 基座与完整第二世界着色预览通路（2026-03-20）
>
...
8. 当前已完成 Phase 1 到 Phase 4.5；真正剩余的大块工作集中在 Phase 5（WASM backend）、Phase 6（schema 资源）和 Phase 7（Query API）。
...
1. **Phase 5 / Week 5：WasmVM + wasm_compiler + hot reload**
   - 这是“AI 能不能真正写游戏行为”的主干任务。

2. **Phase 6 / Week 6：schema:// 资源**
   - 这是“AI 能不能稳定按正确 JSON/结构写东西”的前置护栏。

3. **Phase 7 / Week 7：Query API**
   - 这是“AI 能不能低成本验证自己修改结果”的验证层。

如果只让实习生带着这份文档做一件事，默认应该从 Phase 5 开始，而不是再回头重做 Phase 1 到 Phase 4。
...
4. Guest 侧 panic 能结构化回传 `script_id` / `message` / `source_location`（若可获得）。
...
- [ ] 编译错误与运行时错误可结构化上报
- [ ] Guest 模板中的 `panic` 注入与 `host_report_panic` 通路打通
