...
M5 is complete. M6 currently includes native Jolt regular-grid cloth, arbitrary triangle-surface soft bodies, and pre-tetrahedralized volumetric assets; fixed points; internal edge and volume constraints; pressure/damping settings; revision-cached imported geometry and UVs; incremental synchronization; Scene v2 and Manifest v5 round-tripping; a dedicated deformed-vertex stream; real GPU mesh uploads; integration across every mesh render path and shadow bounds; static/dynamic-body/character collision acceptance; Editor parameter/fixed-point editing, live constraint visualization, and upload profiling; and 64×64 plus multi-instance performance gates. Soft-body self-collision and automatic runtime tetrahedralization remain for later slices.
