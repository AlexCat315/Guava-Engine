...
The first M7 release uses offline pre-fractured assets. `DestructibleAssetBaker` accepts closed convex fragment meshes, density, and local transforms. It sorts by fragment ID, generates stable `asset#convex:<fragmentID>` geometry resource IDs, and computes mass from closed-mesh volume times density. When explicit edges are absent, the importer filters candidates with transformed AABBs and then generates a stable graph from vertex-to-triangle surface distance under a configurable tolerance. Explicit edges take precedence, and automatic generation can be disabled. Duplicate IDs, invalid connections or tolerance, non-finite vertices, out-of-range indices, and zero-volume geometry fail explicitly; no fallback box is generated. An initial graph must be fully connected or completely edge-free: partially connected graphs report unreachable fragments in fragment-ID order during baking, and runtime validation rejects invalid graphs injected around the baker. A bake result installs the geometry and destructible asset resources together; reinstalling a smaller revision removes retired geometry only from that asset's `#convex:` namespace while preserving unrelated shared geometry.
...
Connection edges can override damage or impulse thresholds; zero inherits the source threshold. A non-forced command with `worldPoint` treats the world-space midpoint between its endpoint fragment origins as the edge anchor and breaks only the nearest eligible edge. An incremental break that does not yet separate the graph emits a `connectionBreak` event with empty fragment arrays. A command without a hit point represents global or area damage and breaks every eligible edge. Distance ties use connection ID, and accumulated damage saturates at a finite value. Runtime processing uses stable EntityID, command-index, fragment-ID, and connection-ID order. When the graph separates and every fragment supplies an independent `renderMesh`, the island containing the lowest fragment ID remains on the source: its Collider is rebuilt as a Compound from fragment-local transforms, the complete source mesh is hidden, and runtime child proxies render retained pieces. Detached islands activate as dynamic fragments in fragment-ID order and their internal edges are broken as well. A later forced command or source-level threshold releases only the retained root island, never recreating fragments already released or dropped by a budget. Compatible assets without per-fragment meshes still use complete activation to avoid overlapping the whole mesh with fragment visuals. Arbitrary runtime Boolean cutting remains outside this roadmap.

Global `DestructionSettingsResource` limits active fragments and per-frame events. Fragment budgets truncate by lowest fragment ID and report the dropped count. Fragments recycle by lifetime, continuous sleeping duration, or source removal. `DestructionEventFrameResource` reports fracture, failure, recycle, and overflow; `DestructionStateFrameResource` also distinguishes partial from complete fracture and exposes released, root-retained, and active fragment IDs. Scene v2 and Prefab v2 use asset semantics: dynamic fragments and `DestructibleRetainedFragment` proxies are filtered, while the source body, collider, and visible mesh are restored from the pre-fracture snapshot; neither runtime fragment nor proxy may be a Prefab root. GameSave v2 uses live-snapshot semantics: it preserves accumulated damage, broken edges, partial/complete state, released IDs, the Compound source Collider, proxy hierarchy, relocatable fragment ownership, rigid-body velocities, pending forces and impulses, sleeping state, and remaining lifetime and sleeping-recycle budgets. The source `Destructible` policy also round-trips through Editor Manifest v5.

The unified `PhysicsDebugFrameResource` emits destruction-edge world endpoints, broken state, and source-fractured state in source-EntityID and connection-ID order. Selecting a `Destructible` in the Editor consumes that frame directly: intact edges and midpoint anchors are green, broken ones are red, and each entity is bounded to 4,096 displayed connections. The Inspector edits the full policy and displays asset fragment/connection counts plus current active/broken state. AI scene semantics expose the asset ID, thresholds, budget, and runtime summary. The `destruction-fragments` benchmark deterministically activates complete pre-fractured assets; `destruction-islands` retains one root fragment per source and releases the rest, additionally gating partial/complete source counts, Compound roots, render proxies, and retained-fragment counts. Both independently check activation latency, fixed-step percentiles, memory growth, active count, and dropped substeps, and macOS CI runs their 4,096-fragment configurations in Release mode.
...
- Accumulated destruction damage, partial/complete state, broken connections, released/root fragment IDs, and active-fragment ownership participate in checkpoint hashes; destruction commands are recorded and replayed.
...
- The `cloth-64`, `soft-body-instances`, `destruction-fragments`, and `destruction-islands` benchmarks independently gate one self-colliding 64×64 cloth, eight 32×32 instances, bulk complete fracture, and incremental connected-island release.

## Current boundary

M5, M6, and M7 are complete. M7's production boundary is offline pre-fracture: import supplies closed convex fragments, per-fragment render resources, and a connection graph; runtime owns localized/global damage and impulse thresholds, incremental edge breaking, root-island Compound rebuilding, deterministic island release, budgets, recycling, events, command replay, Editor support, and AI semantics. Assets without per-fragment rendering or transforms representable as Collider TRS fall back to complete activation. Arbitrary runtime Boolean cutting remains out of scope.
