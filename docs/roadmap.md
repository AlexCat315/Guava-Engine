...
> 此节记录截至 2026-05 的现状与优先级排序，指导下阶段实现顺序。
>
> ⚠️ **本节为 2026-05 快照，部分已过时**（粒子 / NavMesh / Prefab 现已有雏形）。
> 动画 / 物理 / 音频 / 粒子 / 脚本五大系统的最新逐项代码级审计与可执行路线图，见
> [gameplay-systems-gap-and-roadmap.md](gameplay-systems-gap-and-roadmap.md)（2026-06-14）。

### 现状总结

| 系统 | 现状 | 优先级 |
|------|------|--------|
| 物理模拟（JoltPhysics） | ✅ 完整：RigidBody / Collider / 约束；播放模式已连线 | — |
| 场景编辑 / ECS | ✅ 完整：Entity + Component；MCP 可控 | — |
| Mesh 渲染 | ✅ GLTF 导入 + wgpu 渲染管线 | — |
| 阴影（Shadow Map） | ✅ 方向光级联阴影（CSM）；点光 / 聚光灯 Atlas | — |
| **音频运行时** | ✅ 已实现：AudioSource 组件 + AudioEngine（平台无关逻辑）+ 可换 `AudioBackend`（AVFoundation / SDL3 / Silent），SceneRuntime tick 驱动 | ~~P0~~ |
| **游戏输入系统** | ✅ 已实现：InputActionMap / InputBinding / InputFrameState；ScriptContext.input；Scancode 常量 | ~~P0~~ |
| **骨骼动画播放** | ✅ 已实现：AnimationRuntime + JointPaletteMap + WGSL skinning + GPU storage buffer | ~~P1~~ |
| **游戏内 UI** | ✅ 已实现：InGameCanvas + InGameUIRuntime（DrawListRenderer 覆盖层）；ctx.drawUI { } 脚本 API | ~~P1~~ |
| **Undo / Redo** | 🟡 SceneRuntime 值类型支持快照，未建栈 | **P2** |
| **Save / Load 游戏状态** | 🟡 场景文件可保存，运行时游戏状态序列化无 | **P2** |
| **独立游戏播放器 / 导出** | 🟡 无独立 GameRuntime，无打包流程 | **P2** |
| Prefab / 预制体 | ⚪ 无 | P3 |
| 粒子系统 | ⚪ 无 | P3 |
| NavMesh / 寻路 | ⚪ 无 | P3 |
...
#### P0-A：音频运行时
- 新建 `Engine/Sources/AudioRuntime/` SwiftPM target
- `AudioSource` 组件：`clipName: String`, `volume: Float`, `loop: Bool`, `playOnAwake: Bool`
- `AudioClipResource`：包装 CoreAudio `AudioFileID` + `ExtAudioFileRef`
- `AudioEngine`（单例）：`AVAudioEngine` 或直接 `AUGraph`；BGM 走 `AVAudioPlayerNode`，SFX 走池化 buffer
- 引擎 tick 中驱动 `playOnAwake` 实体开始播放
- MCP 扩展：`set_audio_source` op（clip / volume / loop / play / stop）

#### P0-B：游戏输入系统
- `InputActionMap`：`[ActionName: [InputBinding]]`，支持键盘 / 鼠标 / 手柄
- `InputRuntime`：每帧轮询 `NSEvent` / `GCController`，派发 `ActionEvent`
- 脚本侧：`Input.isPressed("Jump")`、`Input.axis("MoveX")` 等 API
- 编辑器 Input Settings 面板（M4 后期）

#### P1-A：骨骼动画播放
- `AnimationPlayer` 组件：`clip: String`, `speed: Float`, `loop: Bool`
- `AnimationRuntime`：读 GLTF `AnimationClip`，线性插值 bone transforms，写入 `JointPalette`
- 渲染器：顶点着色器消费 `JointPalette`（skin matrix）

#### P1-B：游戏内 UI
- 实现 `InGameUIProviding`：在 wgpu surface 上覆盖 GuavaUI 树
- 基础组件：Label / Button / ProgressBar / Image
- Canvas 坐标系：归一化设备坐标 → 像素坐标映射

#### P2：Undo / Redo
- `UndoStack`：`[SceneRuntime]` 快照环形缓冲（限 64 帧，约 ~50 MB）
- 每次 `TransactionIR` 提交前 push snapshot
- Ctrl+Z / Ctrl+Shift+Z 触发 pop

### 当前推进目标

**已完成：P0-A 音频运行时**
- `AudioRuntime` SwiftPM target
- `AudioSource` 组件（SceneRuntime）+ `AudioClipResource` + `AudioEngine`（AVAudioEngine）
- SimulationThread tick 驱动；play mode stop 时调用 `resetPlaybackState()`
- MCP `set_audio_source` op；场景快照含 audio 字段

**已完成：P0-B 游戏输入系统**
- `InputActionMap` / `InputBinding` / `InputFrameState` 资源（SceneRuntime）
- `Scancode` 常量（A-Z / 数字 / 方向键 / 功能键）
- `InputStateProcessor`（ScriptRuntime）：每帧处理 InputFrameResource.events → InputFrameState
- `ScriptContext.input`：脚本中直接访问
...
map.bind("MoveZ",  to: .keyAxis(negative: Scancode.w, positive: Scancode.s))
map.bind("Attack", to: .mouseButton(.left))
scene.setResource(map)

// 在脚本 onTick 里
runtime.register("Player") {
    Script().onTick { ctx in
        if ctx.input.isJustPressed("Jump") { /* jump */ }
        let moveX = ctx.input.axis("MoveX")
        ctx.translate(by: [moveX * speed * Float(ctx.deltaTime), 0, 0])
    }
}
```

**已完成：P1-A 骨骼动画播放**
- `AnimationPlayer` 组件（SceneRuntime）+ `AnimationRuntime`（ScriptRuntime）
- GLTF animation clip 评估：TRS 线性插值 + 四元数 slerp + 节点世界矩阵链
- `JointPaletteMap` scene 资源 → `RenderPacket` → per-entity GPU storage buffer
- `mesh.wgsl` binding 8 skinning；非蒙皮网格用 identity fallback 统一走同一 pipeline

**已完成：P1-B 游戏内 UI**
- `InGameCanvas`（EngineKernel）：per-frame 即时模式画布，label / rect / progressBar 命令
- `InGameUIRuntime`（GuavaUIRuntime）：实现 `InGameUIProviding`，3D pass 结束后以 loadOp=.load 开新 pass 叠加 UI
- `ctx.drawUI { canvas in ... }` 脚本 API
- 字体：FontProvider + FontAtlas + TextShaper（系统字体 `.AppleSystemUIFont`）自动 atlas 上传

用法示例：
```swift
runtime.register("HUD") {
    Script().onTick { ctx in
        let score = ctx.resource(GameState.self)?.score ?? 0
        let hp    = ctx.resource(GameState.self)?.health ?? 100
        ctx.drawUI { canvas in
            canvas.label("Score: \(score)", x: 10, y: 10, fontSize: 20)
            canvas.progressBar(x: 10, y: 40, w: 200, h: 16,
                               value: Float(hp), maxValue: 100)
            canvas.rect(x: 10, y: 64, w: 200, h: 2,
                        color: .gray(0.3, alpha: 0.6))
        }
    }
}
```
