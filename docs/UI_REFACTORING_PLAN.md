...
│  [□ Grid][◇ Snap▼]                  │ [▶ Play][⏸ Pause][⏹] │
├──────────┬────────────────────────────────┬─────────────────┤
│ Outliner │        Viewport                │   Details       │
│ (层级树) │      (3D 场景)                 │   (属性面板)    │
│          │                                │                 │
│ [搜索]   │  [View▼][Show▼][Lighting▼]     │  [Entity Name]  │
│ Entity   │                        [ViewCube]│ ▼ Transform    │
│ Entity   │  FPS: 60  Entities: 42         │   Location X[]  │
│ Entity   │                                │   Location Y[]  │
│          │                                │   Location Z[]  │
├──────────┴────────────────────────────────┴─────────────────┤
│  Content Browser (底部, 可折叠)                              │
├─────────────────────────────────────────────────────────────┤
│  Output Log (底部标签页)                                     │
└─────────────────────────────────────────────────────────────┘
```
...
通过 `src/editor/ui/layout.zig` 中的 DockBuilder 系统调整面板位置：

- **Outliner**（原 Hierarchy）：保持在左侧
- **Viewport**：居中，占据最大空间
- **Details**（原 Inspector）：保持在右侧
- **Content Browser**：移至底部，可折叠
- **Global Toolbar**：从视口内分离为全局工具栏
...
| 阶段 | 涉及文件数 | 改动行数 | 预估时间 |
|------|-----------|----------|----------|
| 1. 主题系统 | 2 | +400 / -50 | 2-3 天 |
| 2. 样式初始化 | 1 | +50 / -50 | 0.5 天 |
| 3. 布局重构 | 3 | +200 / -100 | 2 天 |
| 4. Viewport | 1 | +300 / -200 | 3-4 天 |
| 5. Outliner | 1 | +150 / -100 | 2 天 |
| 6. Inspector | 1 | +200 / -150 | 3 天 |
| 7. 图标系统 | 2 | +100 / -30 | 1-2 天 |
| 8. 交互改进 | 5 | +200 / -50 | 2-3 天 |
| **总计** | **~15** | **~1600 / -730** | **~15-20 天** |
