...
Guava 要解决的问题不是“让 AI 会调工具”，而是让 AI 对结构良好的工程数据，**不依赖视觉**也能直接理解：
...
对于结构薄弱的资产（单 mesh、无命名、无 rig），纯几何分析不足以单独支撑功能语义。这种情况下视觉后端是 `SemanticAnalyzer` 的事实主力之一，不再是辅助。详见 `ai-native-semantic-pipeline-design.md` 第 0、2.5 节。
...
- 结构良好资产（命名、分组、材质分区、rig、custom metadata 至少具备其一）：纯文档层即可支撑
- 结构薄弱资产：必须同时启用 `ai-native-semantic-pipeline-design.md` 中的 VisionBackend / CategoryPriorBackend，才能从匿名 region 提升到功能语义
...
`ModelSemanticSummary` 区分两类内容：

- `structural_parts`：来自 RawStructure（命名、分组、材质槽、rig），属于 authored truth
- `inferred_parts`：来自 SemanticAnalyzer 的候选结果，必须带 `confidence` 与 `source` 标签

`ModelSemanticSummary` 应回答：

- 这是一个什么模型
- 有多少 part（区分 structural / inferred）
- 主要 part 如何组织
...
2. 导入分析
3. 作者标注
4. 历史操作统计与 `SemanticMemoryStore` 命中（按几何指纹寻址，资产重导入仍可复用）
5. AI 推断（标记为 inferred，不能覆盖 authored truth）
   - 在结构薄弱资产上，推断的实际承担者是 VisionBackend 与 CategoryPriorBackend
...
`DCC / Asset` → `Importer` → `StructureExtractor` → `GeometryAnalyzer` → `CandidateRegionBuilder` → `GeometryFingerprinter` → `SemanticAnalyzer` → `AmbiguityScorer` → (可选) `MinimalConfirmationUI` → `SemanticMemoryStore` → `ModelDocument`

完整模块定义、字段、接口、失败模式与降级路径见 `ai-native-semantic-pipeline-design.md`。

这里的关键约束是：

- `ModelDocument` 保存结果
- `SemanticAnalyzer` 是多后端可插拔接口（NameHeuristic / Rig / Metadata / Memory / Vision / CategoryPrior），无视觉时自动退化
- `AmbiguityScorer` 决定何时打断用户，阈值在 `ProjectDocument` 中可配置
- `GeometryFingerprinter` 让 `SemanticMemoryStore` 跨重导入仍能命中，避免身份漂移
- `MinimalConfirmationUI` 只在歧义点要求用户给最少信号
- 用户不应被迫先做一套完整手工标注
...
打断时机由 `AmbiguityScorer` 决定，不由提案数量或字段数量决定。判定信号包括：

- 单 backend confidence 是否达到 `auto_threshold`
- 多 backend 是否冲突（top1 与 top2 差小于 `conflict_margin`）
- 操作是否涉及破坏性 capability（topology edit、asset 本体改写）
- region 是否已被 `SemanticMemoryStore` 命中

只有判定为 NeedsConfirmation 时，才要求用户做最小确认：
...
同一批次的歧义应在一次 UI 中问完，避免反复打断。Guava 不应要求用户先做一整套手工语义标注。
...
记忆按 `GeometryFingerprint` 寻址而非按 region 索引，因此同一资产重导入（拓扑微调内）或同族资产再次出现，AI 都不需要从零猜一次。指纹算法、匹配阈值与失败模式见 `ai-native-semantic-pipeline-design.md` 第 3 节。
...
- **结构良好资产**：视觉是辅助手段，主理解面是 canonical document
- **结构薄弱资产**：视觉是 `SemanticAnalyzer` 的事实主力之一（VisionBackend），从匿名 region 提升到功能语义这一步无法只靠几何完成

两种情况下，视觉产出的语义都标记为 `inferred`，必须经过 `AmbiguityScorer` 与（必要时）用户确认才能写入 `ModelDocument`。
...
- 结构良好资产：视觉是辅助手段，语义文档是主理解面（随着未来视觉能力的增强，视觉会被提升主力）
- 结构薄弱资产：视觉是语义生产的事实主力之一，与几何分析协作产出 inferred 标签
- 审美判断与语义控制始终协作，而不是互相替代
...
2. **视觉反馈层**
   ...
   - 根据语义控制层的判断，二次锁定和确认哪些区域可能对应耳朵、眼睛、尾巴
   - 判断“哪里不好看”
...
详细模块定义、字段、接口、失败与降级策略见 `ai-native-semantic-pipeline-design.md`。本节只列总纲层模块清单：

- `StructureExtractor`
- `GeometryAnalyzer`
- `CandidateRegionBuilder`
- `GeometryFingerprinter`
- `SemanticAnalyzer`（多后端：NameHeuristic / Rig / Metadata / Memory / Vision / CategoryPrior）
- `AmbiguityScorer`
...
12. `Context Memory Index` 详细设计 → 已落地：`ai-native-context-memory-index-design.md`
