...
private struct CapabilityOperationProjection {
    ...
    private init(sceneMutation: SceneMutation) {
        ...
        case let .setParticleEmitter(_, emitter):
            ...
            self.arguments["angular_velocity"] = .number(Double(emitter.angularVelocity))
            self.arguments["angular_velocity_randomness"] = .number(Double(emitter.angularVelocityRandomness))
