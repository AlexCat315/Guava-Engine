...
struct GuavaJoltContextImpl {
    ...
    bool destroy_soft_body(uint64_t entity) {
        auto existing = soft_body_ids.find(entity);
        if (existing == soft_body_ids.end()) return false;
        JPH::BodyInterface& bi = physics_system.GetBodyInterface();
        bi.RemoveBody(existing->second);
        bi.DestroyBody(existing->second);
    ...
    bool create_soft_body(const GuavaJoltSoftBodyDesc& desc) {
        ...
        const uint32_t vertex_count = static_cast<uint32_t>(vertex_count64);
        std::unordered_set<uint32_t> fixed_vertices;
        fixed_vertices.reserve(desc.fixed_vertex_count);
        for (uint32_t index = 0; index < desc.fixed_vertex_count; ++index) {
            if (desc.fixed_vertices[index] >= vertex_count) {
                last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
                return false;
            }
            fixed_vertices.insert(desc.fixed_vertices[index]);
        }
        ...
        const JPH::SoftBodySharedSettings::VertexAttributes attributes(
            std::max(0.0f, desc.compliance),
            std::max(0.0f, desc.shear_compliance),
            std::max(0.0f, desc.bend_compliance));
        const JPH::SoftBodySharedSettings::EBendType bend_type =
            desc.bend_type == 2
        ...
        JPH::SoftBodyCreationSettings settings(
            ...
            normalized_quat(desc.rotation_x, desc.rotation_y, desc.rotation_z, desc.rotation_w),
            Layers::MOVING);
        settings.mUserData = desc.entity_id;
        settings.mNumIterations = std::max<uint32_t>(1, std::min<uint32_t>(desc.solver_iterations, 128));
        settings.mLinearDamping = std::max(0.0f, desc.linear_damping);
        settings.mMaxLinearVelocity = std::max(0.0f, desc.max_linear_velocity);
        settings.mRestitution = JPH::Clamp(desc.restitution, 0.0f, 1.0f);
        settings.mFriction = std::max(0.0f, desc.friction);
        settings.mPressure = std::max(0.0f, desc.pressure);
        settings.mGravityFactor = desc.gravity_factor;
        settings.mVertexRadius = std::max(0.0f, desc.vertex_radius);
        settings.mAllowSleeping = desc.allow_sleep != 0;
        settings.mFacesDoubleSided = desc.faces_double_sided != 0;

        JPH::BodyInterface& bi = physics_system.GetBodyInterface();
        const JPH::BodyID body_id = bi.CreateAndAddSoftBody(
    ...
    bool sync_soft_bodies(
        ...
        *out_stats = GuavaJoltSoftBodySyncStats{};

        std::unordered_map<uint64_t, const GuavaJoltSoftBodyDesc*> incoming;
        incoming.reserve(upsert_count);
        for (size_t index = 0; index < upsert_count; ++index) {
            incoming[upserts[index].entity_id] = &upserts[index];
        }
        std::vector<uint64_t> stale;
        if (full_snapshot) {
            for (const auto& entry : soft_body_ids) {
                if (incoming.find(entry.first) == incoming.end()) stale.push_back(entry.first);
            }
        } else if (removal_count > 0) {
            stale.assign(removals, removals + removal_count);
        }
        std::sort(stale.begin(), stale.end());
        stale.erase(std::unique(stale.begin(), stale.end()), stale.end());
        for (uint64_t entity : stale) {
            if (destroy_soft_body(entity)) ++out_stats->removed_soft_bodies;
        }

        std::vector<uint64_t> ids;
        ids.reserve(incoming.size());
        for (const auto& entry : incoming) ids.push_back(entry.first);
        std::sort(ids.begin(), ids.end());
        for (uint64_t entity : ids) {
            if (body_ids.find(entity) != body_ids.end()) {
                last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
                return false;
            }
            if (destroy_soft_body(entity)) ++out_stats->removed_soft_bodies;
            if (!create_soft_body(*incoming[entity])) return false;
            ++out_stats->synchronized_soft_bodies;
        }
        return true;
    }

    uint32_t copy_soft_body_states(
        ...
        if (!out_vertex_count
            ...
            || (vertex_capacity > 0 && !out_positions_xyz)) {
            last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
            return 0;
        }
        *out_vertex_count = 0;
        std::vector<uint64_t> ids;
        ids.reserve(soft_body_ids.size());
        for (const auto& entry : soft_body_ids) ids.push_back(entry.first);
        std::sort(ids.begin(), ids.end());
        if (ids.size() > state_capacity) {
            last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
            return 0;
        }
        ...
        for (uint64_t entity : ids) {
            JPH::BodyLockRead lock(
                physics_system.GetBodyLockInterface(), soft_body_ids[entity]);
            if (!lock.Succeeded() || !lock.GetBody().IsSoftBody()) {
                last_error = GUAVA_JOLT_ERROR_UPDATE_FAILED;
                return 0;
            }
            const auto* properties = static_cast<const JPH::SoftBodyMotionProperties*>(
                lock.GetBody().GetMotionProperties());
            required_vertices += properties->GetVertices().size();
        }
        if (required_vertices > vertex_capacity || required_vertices > UINT32_MAX) {
            last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
            return 0;
        }
        ...
        const JPH::BodyInterface& bi = physics_system.GetBodyInterface();
        for (uint64_t entity : ids) {
            JPH::BodyLockRead lock(
                physics_system.GetBodyLockInterface(), soft_body_ids[entity]);
            ...
            GuavaJoltSoftBodyState& state = out_states[state_index++];
            state = GuavaJoltSoftBodyState{};
            state.entity_id = entity;
            state.vertex_offset = static_cast<uint32_t>(vertex_offset);
            state.vertex_count = static_cast<uint32_t>(properties->GetVertices().size());
            state.is_sleeping = bi.IsActive(soft_body_ids[entity]) ? 0 : 1;
            const JPH::RMat44 center_of_mass = body.GetCenterOfMassTransform();
            for (const JPH::SoftBodyVertex& vertex : properties->GetVertices()) {
                const JPH::RVec3 position = center_of_mass * vertex.mPosition;
                out_positions_xyz[vertex_offset * 3] = static_cast<float>(position.GetX());
                out_positions_xyz[vertex_offset * 3 + 1] = static_cast<float>(position.GetY());
                out_positions_xyz[vertex_offset * 3 + 2] = static_cast<float>(position.GetZ());
                ++vertex_offset;
            }
        }
        *out_vertex_count = static_cast<uint32_t>(vertex_offset);
        return static_cast<uint32_t>(state_index);
    }
    ...
    bool sync_vehicles(
        ...
        for (const GuavaJoltVehicleDesc* desc_ptr : ordered) {
            ...
            for (uint32_t i = 0; i < desc.track_count; ++i) {
                ...
                if (track.wheel_count == 0 || !track.wheels
                    || track.driven_wheel < 0
                    || track.driven_wheel >= static_cast<int32_t>(desc.wheel_count)) {
                    last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
                    return false;
                }
                bool includes_driven_wheel = false;
                for (uint32_t j = 0; j < track.wheel_count; ++j) {
                    const int32_t wheel = track.wheels[j];
                    if (wheel < 0 || wheel >= static_cast<int32_t>(desc.wheel_count)) {
                        last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
                        return false;
                    }
                    includes_driven_wheel |= wheel == track.driven_wheel;
                }
                if (!includes_driven_wheel) {
                    last_error = GUAVA_JOLT_ERROR_INVALID_ARGUMENT;
                    return false;
                }
            }
            ...
            for (uint32_t i = 0; i < desc.wheel_count; ++i) {
                ...
                JPH::WheelSettings* wheel = nullptr;
                if (desc.controller_type == 1) {
                    auto* tracked_wheel = new JPH::WheelSettingsTV;
                    tracked_wheel->mLongitudinalFriction = std::max(
                        0.0f, desc.tracked_longitudinal_friction);
                    tracked_wheel->mLateralFriction = std::max(
                        0.0f, desc.tracked_lateral_friction);
                    wheel = tracked_wheel;
                } else {
                    auto* wheeled_wheel = new JPH::WheelSettingsWV;
                    wheeled_wheel->mInertia = std::max(0.001f, source.inertia);
                    wheeled_wheel->mAngularDamping = std::max(0.0f, source.angular_damping);
                    wheeled_wheel->mMaxSteerAngle = std::max(0.0f, source.max_steer_angle);
                    wheeled_wheel->mMaxBrakeTorque = std::max(0.0f, source.max_brake_torque);
                    wheeled_wheel->mMaxHandBrakeTorque = std::max(
                        0.0f, source.max_hand_brake_torque);
                    wheel = wheeled_wheel;
                }
            ...
            auto configure_powertrain = [&](auto* controller) {
                ...
                controller->mEngine.mMaxRPM = std::max(
                    controller->mEngine.mMinRPM, desc.engine_max_rpm);
                controller->mEngine.mInertia = std::max(0.001f, desc.engine_inertia);
                controller->mEngine.mAngularDamping = std::max(
                    0.0f, desc.engine_angular_damping);
                ...
                transmission.mClutchReleaseTime = std::max(
                    0.0f, desc.transmission_clutch_release_time);
                transmission.mSwitchLatency = std::max(
                    0.0f, desc.transmission_switch_latency);
                ...
                transmission.mClutchStrength = std::max(
                    0.0f, desc.transmission_clutch_strength);
            };
            auto configure_differentials = [&](auto* controller) {
                ...
                for (uint32_t i = 0; i < desc.differential_count; ++i) {
                    ...
                    differential.mDifferentialRatio = std::max(
                        0.001f, source.differential_ratio);
                    differential.mLeftRightSplit = JPH::Clamp(
                        source.left_right_split, 0.0f, 1.0f);
                    differential.mLimitedSlipRatio = std::max(
                        1.0001f, source.limited_slip_ratio);
                    differential.mEngineTorqueRatio = std::max(
                        0.0f, source.engine_torque_ratio);
                    controller->mDifferentials.push_back(differential);
                }
            };
            if (desc.controller_type == 1) {
                JPH::Ref<JPH::TrackedVehicleControllerSettings> controller =
                    new JPH::TrackedVehicleControllerSettings;
                configure_powertrain(controller.GetPtr());
                for (uint32_t track_index = 0; track_index < 2; ++track_index) {
                    const GuavaJoltVehicleTrackDesc& source = desc.tracks[track_index];
                    JPH::VehicleTrackSettings& track = controller->mTracks[track_index];
                    track.mDrivenWheel = static_cast<JPH::uint>(source.driven_wheel);
                    track.mWheels.clear();
                    for (uint32_t i = 0; i < source.wheel_count; ++i) {
                        track.mWheels.push_back(static_cast<JPH::uint>(source.wheels[i]));
                    }
                    track.mInertia = std::max(0.001f, source.inertia);
                    track.mAngularDamping = std::max(0.0f, source.angular_damping);
                    track.mMaxBrakeTorque = std::max(0.0f, source.max_brake_torque);
                    track.mDifferentialRatio = std::max(0.001f, source.differential_ratio);
                }
                settings.mController = controller;
            } else if (desc.controller_type == 2) {
                JPH::Ref<JPH::MotorcycleControllerSettings> controller =
                    new JPH::MotorcycleControllerSettings;
                configure_powertrain(controller.GetPtr());
                configure_differentials(controller.GetPtr());
                controller->mMaxLeanAngle = JPH::Clamp(
                    desc.motorcycle_max_lean_angle, 0.0f, 0.5f * JPH::JPH_PI);
                controller->mLeanSpringConstant = std::max(
                    0.0f, desc.motorcycle_lean_spring_constant);
                controller->mLeanSpringDamping = std::max(
                    0.0f, desc.motorcycle_lean_spring_damping);
                controller->mLeanSpringIntegrationCoefficient = std::max(
                    0.0f, desc.motorcycle_lean_spring_integration_coefficient);
                controller->mLeanSpringIntegrationCoefficientDecay = std::max(
                    0.0f, desc.motorcycle_lean_spring_integration_coefficient_decay);
                controller->mLeanSmoothingFactor = JPH::Clamp(
                    desc.motorcycle_lean_smoothing_factor, 0.0f, 1.0f);
                settings.mController = controller;
            } else {
            ...
            if (desc.controller_type == 2) {
                vehicle_constraint->SetVehicleCollisionTester(
                    new JPH::VehicleCollisionTesterCastCylinder(Layers::MOVING, 1.0f));
                auto* motorcycle = static_cast<JPH::MotorcycleController*>(
                    vehicle_constraint->GetController());
                motorcycle->EnableLeanController(
                    desc.motorcycle_enable_lean_controller != 0);
                motorcycle->EnableLeanSteeringLimit(
                    desc.motorcycle_enable_lean_steering_limit != 0);
            } else {
                vehicle_constraint->SetVehicleCollisionTester(
    ...
    bool set_vehicle_commands(const GuavaJoltVehicleCommand* commands, size_t command_count) {
        ...
        std::sort(ids.begin(), ids.end());
        for (uint64_t entity : ids) {
            NativeVehicle& vehicle = vehicles[entity];
            if (vehicle.controller_type == 1) {
                auto* controller = static_cast<JPH::TrackedVehicleController*>(
                    vehicle.constraint->GetController());
                controller->SetDriverInput(0, 1, 1, 0);
            } else {
        ...
        for (size_t i = 0; i < command_count; ++i) {
            ...
            NativeVehicle& vehicle = vehicle_it->second;
            if (vehicle.controller_type == 1) {
                auto* controller = static_cast<JPH::TrackedVehicleController*>(
                    vehicle.constraint->GetController());
                float throttle = JPH::Clamp(command.throttle, -1.0f, 1.0f);
                const float steering = JPH::Clamp(command.steering, -1.0f, 1.0f);
                ...
                if (steering > 0.001f) {
                    if (std::abs(throttle) < 0.001f) {
                        ...
                        right_ratio = std::max(0.2f, 1.0f - 0.8f * steering);
                    }
                } else if (steering < -0.001f) {
                    if (std::abs(throttle) < 0.001f) {
                        throttle = -steering;
                        left_ratio = -1.0f;
                    } else {
                        left_ratio = std::max(0.2f, 1.0f + 0.8f * steering);
                    }
                }
                controller->SetDriverInput(
                    ...
                    left_ratio,
                    right_ratio,
                    std::max(JPH::Clamp(command.brake, 0.0f, 1.0f),
    ...
    bool entity_for_body_id(JPH::BodyID body_id, uint64_t& out_entity) const {
        if (body_id.IsInvalid()) return false;
        for (const auto& entry : body_ids) {
            if (entry.second == body_id) {
                out_entity = entry.first;
                return true;
            }
        }
        for (const auto& entry : soft_body_ids) {
    ...
    uint32_t copy_vehicle_states(
        ...
        for (uint64_t entity : ids) {
            ...
            for (uint32_t wheel_index = 0; wheel_index < state.wheel_state_count; ++wheel_index) {
                ...
                if (wheel->HasContact()) {
                    uint64_t contact_entity = 0;
                    output.has_contact_entity = entity_for_body_id(
                        wheel->GetContactBodyID(), contact_entity) ? 1 : 0;
...
bool guava_jolt_bridge_get_abi_layout(GuavaJoltABILayout* out_layout) {
    ...
    out_layout->soft_body_desc_size = static_cast<uint32_t>(sizeof(GuavaJoltSoftBodyDesc));
    out_layout->soft_body_state_size = static_cast<uint32_t>(sizeof(GuavaJoltSoftBodyState));
    out_layout->soft_body_sync_stats_size = static_cast<uint32_t>(sizeof(GuavaJoltSoftBodySyncStats));
    return true;
}
...
bool guava_jolt_bridge_get_vehicle_abi_layout(GuavaJoltVehicleABILayout* out_layout) {
    ...
    out_layout->track_desc_size = static_cast<uint32_t>(sizeof(GuavaJoltVehicleTrackDesc));
...
bool guava_jolt_context_sync_soft_bodies(
    ...
    GuavaJoltSoftBodySyncStats* out_stats) {
    if (!context) return false;
    return context->sync_soft_bodies(
        upserts, upsert_count, removals, removal_count, full_snapshot, out_stats);
}
...
uint32_t guava_jolt_context_copy_soft_body_states(
    ...
    uint32_t* out_vertex_count) {
    if (!context) return 0;
    return context->copy_soft_body_states(
        out_states, state_capacity, out_positions_xyz, vertex_capacity,
        out_vertex_count);
}
