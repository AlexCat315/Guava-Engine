...
/// Minimal, dependency-free PNG (RGBA8) encoder for test fixtures (zlib
/// "stored" blocks — no compression library, no platform image framework).
/// The native bridge's stb_image decoder reads it back identically on every
/// platform. Mirrors the copy in AssetPipelineTests (test targets can't share).
enum PortablePNG {
